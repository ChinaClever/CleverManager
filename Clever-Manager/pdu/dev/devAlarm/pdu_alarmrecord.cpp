...
#include "storage/cloudalarmlog.h"
#include "services/dingtalk/dt_dingtalk.h"
...
PDU_AlarmRecord::PDU_AlarmRecord(QObject *parent) : QThread(parent)
{
    ...
    //(this, SIGNAL(alarmDingSig(QString)),DT_DingTalk::bulid(this), SLOT(alarmDingSlot(QString))); // 报警提示框
    //mAlarmRun = new PDU_AlarmMusicRun();
    //connect(this, SIGNAL(alarmPlaySig()) , mAlarmRun ,SLOT(alarmPlaySlot()));
}
    ...
    for(int i=0; i<size; ++i)
    {
        ...
        if(value > 0) // 已经报警
        {
            ...
            if(value != 1) // 需要记录
            {
                ...
                QString minStr = "---";
                value = unit->min->get(i);
                if(value >= 0) minStr = QString::number(value/10.0);

                QString maxStr = "---";
                value = unit->max->get(i);
                if(value >= 0) maxStr = QString::number(value/10.0);

                QString currentCirInfoStr = QString(tr("  最小值：%1%2   最大值：%3%4").arg(minStr).arg(str).arg(maxStr).arg(str));
...
{
    if(!getAlarmSignal())//打开报警和关闭报警 2019/7/24 by peng
    {
        if(isOpen == false)
        {
            isOpen = true;
...
void PDU_AlarmRecord::sentEmail(void)
{
    ...
    QString str = tr("检测到PDU设备报警，%1 设备名：%2，设备IP：%3，").arg(salve).arg(name).arg(ip) + "\n";
    str += smsMsg;
