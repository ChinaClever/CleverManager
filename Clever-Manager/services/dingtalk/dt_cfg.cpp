...
void DT_Cfg::read()
{
    ...
    cfg->webhook = Sys_ConfigFile_ReadStr(str, prefix);
    ...
    cfg->key = Sys_ConfigFile_ReadStr(str, prefix);
    if(cfg->key.isEmpty()) {
    ...
    ret = Sys_ConfigFile_ReadInt(str, prefix);
    if(ret <= 0)  ret = 0;
    cfg->isAtAll = ret;
...
void DT_Cfg::write()
{
    ...
    QString prefix = getPrefix();
    QString str = QString("%1_webhook").arg(prefix);
    Sys_ConfigFile_Write(str, cfg->webhook, prefix);

    str = QString("%1_key").arg(prefix);
    Sys_ConfigFile_Write(str, cfg->key, prefix);

    str = QString("%1_mob").arg(prefix);
    Sys_ConfigFile_Write(str, cfg->mobiles.at(0), prefix);

    int rtn = 0;
    if(cfg->en) rtn = 1;
    str = QString("%1_en").arg(prefix);
    Sys_ConfigFile_Write(str, QString::number(rtn), prefix);
