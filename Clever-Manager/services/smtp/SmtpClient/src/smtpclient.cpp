...
bool SmtpClient::connectToHost()
{
    ...
    try
    {
        ...
        if (connectionType == TlsConnection) {
            ...
            //((QSslSocket*) socket)->setProtocol(QSsl::TlsV1_1);//蓝思科技支持TLS v1.1 TlsConnection AuthLogin
            ((QSslSocket*) socket)->startClientEncryption();

            if (!((QSslSocket*) socket)->waitForEncrypted(connectionTimeout)) {
                qDebug() << "waitForEncrypted(connectionTimeout)"<<((QSslSocket*) socket)->errorString()<<endl;
                emit smtpError(ConnectionTimeoutError);
                return false;
            }
            ...
            // The response code needs to be 250.
            if (responseCode != 250) {
                qDebug() << " sendMessage(\"EHLO \" + name);"<<endl;
                emit smtpError(ServerError);
                return false;
            }
        }
...
bool SmtpClient::login(const QString &user, const QString &password, AuthMethod method)
{
    try {
        if (method == AuthPlain)
        {
            ...
            if (responseCode != 235)
            {
                qDebug() << " If the response is not 235 then the authentication was faild"<<endl;
        ...
        else if (method == AuthLogin)
        {
            ...
            waitForResponse();
            if (responseCode != 334) { qDebug() << "Sending command: AUTH LOGIN Wait for 334 response code"<<endl;emit smtpError(AuthenticationFailedError); return false; }
            ...
            // Wait for 334
            waitForResponse();
            if (responseCode != 334) { qDebug() << "LOGIN Send the username in base64 Wait for 334"<<endl;emit smtpError(AuthenticationFailedError); return false; }
            ...
            if (responseCode != 235)
            {
                qDebug() << "LOGIN Send the password in base64,Wait for the server's responce"<<endl;
                emit smtpError(AuthenticationFailedError);
                return false;
            }
        }
        else if (method == AuthNtlm)
        {
            // Sending command: AUTH NTLM
            sendMessage("AUTH NTLM");

            // Wait for 334 response code
            waitForResponse();
            if (responseCode != 334) { qDebug() << "NTLM Sending command: AUTH NTLM Wait for 334 response code"<<endl;emit smtpError(AuthenticationFailedError); return false; }
            ...
            // Wait for 334
            waitForResponse();
            if (responseCode != 334) { qDebug() << "NTLM Send the username in base64 Wait for 334"<<endl;emit smtpError(AuthenticationFailedError); return false; }
            ...
            if (responseCode != 235)
            {
                qDebug() << "NTLM Send the password in base64,Wait for the server's responce"<<endl;
                emit smtpError(AuthenticationFailedError);
                return false;
            }
        }
