    ...
    distory_flag(false)
{
    ...
    m_shortcode_socket = new QTcpSocket(this);
...
void ShortCodeSocket::read_message()
{
    ...
    if(!buffer.isEmpty())
    {
        mShort_code= QString(buffer);
        if(6 == mShort_code.size())
        {
//            MY_DEBUG(1, GREEN) << "short code : " << mShort_code.toStdString() << DEBUG_END;
            distory_flag = true;
            start();
        }
        else
...
void ShortCodeSocket::run()
{
    zebra_client::get_instance()->add_friend(mShort_code, m_zpdu_ip);
}
...
AutoAddFriend::AutoAddFriend()
{
    ...
    m_shortcode_timer->setInterval(20000);
...
{
    QVector<DbZebraChannelItem> qvec = DbZebraChannels::get()->selectItemByChannelName(ip);
