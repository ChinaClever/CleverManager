...
DB_SqlTableModel::DB_SqlTableModel(QWidget *parent) : QWidget(parent)
{
    isRun = false;
...
DB_SqlTableModel::~DB_SqlTableModel()
{
...
bool DB_SqlTableModel::removeRow(int curRow)
{
    ...
    if(ret) {
        if(isRun) {
            ...
            isRun = true;
            QTimer::singleShot(3*1500,this,SLOT(submitChanges()));
        }
    } else
 ...
 * @brief 重新关联表
 */
bool DB_SqlTableModel::refreshTable(const QString &name)
{
