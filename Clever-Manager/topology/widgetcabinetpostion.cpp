#include "widgetcabinetpostion.h"
#include <QPainter>
#include <QPaintEvent>
#include <QHeaderView>
#include "common.h"

WidgetCabinetPostion::WidgetCabinetPostion(QWidget *parent) : QTableWidget(parent),m_currentAddress(-1)
{
    this->verticalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    this->setMouseTracking(true);
}

void WidgetCabinetPostion::setItems(const QVector<ConsumerDeviceItem> &items)
{
    m_items = items;
    QWidget::update();
}

void WidgetCabinetPostion::removeItem(const ConsumerDeviceItem &item)
{
...
bool WidgetCabinetPostion::updateItem(const ConsumerDeviceItem &item)
{
    for(int i=0 ; i < m_items.count() ; ++i){
        if(m_items[i].id == item.id){
...
bool WidgetCabinetPostion::addItem(const ConsumerDeviceItem &item)
{
    if(isEmptyPostion(item)){
        m_items.append(item);
        QWidget::update();
...
ConsumerDeviceItem WidgetCabinetPostion::deviceItem(int address,bool onlyAddress)
{
...
int WidgetCabinetPostion::currentAddress()
{
...
const QVector<ConsumerDeviceItem> &WidgetCabinetPostion::items() const
{
    return m_items;
}

QRect WidgetCabinetPostion::itemRect(const ConsumerDeviceItem& item)
{
...
void WidgetCabinetPostion::paintEvent(QPaintEvent *e)
{
    ...
    foreach(const ConsumerDeviceItem& item,m_items){
        ...
        QColor color = cm::color(item.address);
...
void WidgetCabinetPostion::mouseReleaseEvent(QMouseEvent *e)
{
    m_clickedPos = e->pos();
    QTableWidget::mouseReleaseEvent(e);
}

void WidgetCabinetPostion::mouseMoveEvent(QMouseEvent *e)
{
...
bool WidgetCabinetPostion::isEmptyPostion(const ConsumerDeviceItem &item)
{
...
int WidgetCabinetPostion::updateCurrentAddress(int address)
{
